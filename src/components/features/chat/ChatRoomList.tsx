...
const mockChatRooms: ChatListItemDto[] = [
  {
    userId: '1',
...
const ChatRoomList = (props: ChatRoomListProps) => {
    const fetchPosts = () : Array<ChatListItemDto> => {return mockChatRooms};
    const [rooms] = createSignal<Array<ChatListItemDto>>(fetchPosts());
    ...
    <div class=" w-32 sm:w-64 md:w-80 border-r overflow-y-auto border-gray-200 bg-white h-[calc(100vh-64px)] flex flex-col">
        ...
        <div class="flex-1 shrink-0 min-h-64 overflow-y-auto">
          <For each={filteredRooms()}>
            {(room) => (
              <div onClick={props.onSelectChat}>
                <ChatRoomItem room={room} activeRoomId={activeRoomId} setActiveRoomId={setActiveRoomId} />
              </div>
            )}
          </For>
      </div>
