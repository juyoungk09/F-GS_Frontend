...
const api = {
  async getPost(id: number) {
    const r = await fetch(`${BASE_URL}/posts/${id}`, { credentials: "include" });
    if (!r.ok) throw new Error(await readErr(r));
    return r.json();
  },
  async updatePostJSON(id: number, payload: any) {
    const r = await fetch(`${BASE_URL}/posts/${id}`, {
      method: "PUT",
...
const EditPost = () => {
  ...
  const [post] = createResource<Post>(
    async () => {
      const r = await axios(`${BASE_URL}/posts/${params.id}`, { withCredentials: true });
      return r.data;
    },
  );
  ...
  onMount(async () => {
    ...
    try {
      ...
      setExistingImageUrl(data?.attachments?.[0] ?? null);
    } catch {
