...
const [allTags] = createResource<Tag[]>(async () => {
  ...
  const addQuestion = () =>
    setQuestions(
      produce((qs) => {
        qs.push({ label: '', is_file: false, question_type: 'text', required: false });
      }),
    );
  ...
  const handleComplete = async (e: Event) => {
    ...
    try {
      ...
      if (!deadline().trim() || maxRecruits() < 1) {
        throw new Error('마감일과 모집 인원을 확인해주세요.');
      }
      
      // Validate at least one tag is selected
      if (selectedTags().length === 0) {
        throw new Error('최소 하나 이상의 태그를 선택해주세요.');
      }
      ...
      // Create the post
      const postRes = await axios.post(`${BASE_URL}/posts/private`, {
        title: title() || "",
        content: content() || "",
      ...
      const r = await axios.post(
        `${BASE_URL}/posts/private/tags`,
        { post_id: pid, tag_ids: tagIds },
      ...
      if(imageFile()) {
        ...
        const imageRes = await axios.post(`${BASE_URL}/posts/private/${pid}/attachments`, fd, {
          withCredentials: true,
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        console.log(imageRes.data);
      }
      const questionPost = await axios.post(`${BASE_URL}/posts/private/form`, {
        post_id: pid,
        questions: questions,
      }, {
        withCredentials: true,
  ...
  const canSubmit = createMemo(() => !!title().trim() && !!content().trim() && !!deadline().trim() && maxRecruits() >= 1);
        ...
        <div class="max-w-6xl mx-auto p-6">
          <form onSubmit={handleComplete} class="bg-white rounded-2xl shadow-md p-6 md:p-8">
              ...
              <section class="pr-0 md:pr-8">
                ...
                <div class="space-y-4">
                  ...
                  <div>
                    ...
                    <button 
                    type='button'
                  class="text-gray-900 w-full text-center h-10 bg-primary_color_2 rounded-lg "
                  onClick={(e) => {
                    e.preventDefault(); 
                  ...
                  <Show when={isDropdownOpen()}>
                        <div class="absolute z-10 w-full max-w-[500px] bg-white border border-gray-200 rounded-lg shadow-lg">
            ...
            <div class="mt-8">
              <button
                type="submit"
                class="bg-primary_color_3 text-white px-4 py-3 rounded-lg w-full shadow-md hover:bg-primary_color_2 transition active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed"
                onClick={(e) => {
                  e.preventDefault();
                  ...
                  console.log('maxRecruits:', maxRecruits());
              ...
              {!canSubmit && (
                <div class="mt-2 text-sm text-red-500">
                  {!title().trim() && <div>- 제목을 입력해주세요</div>}
                  {!content().trim() && <div>- 내용을 입력해주세요</div>}
                  {!deadline().trim() && <div>- 마감일을 선택해주세요</div>}
                  {maxRecruits() < 1 && <div>- 모집 인원은 1명 이상이어야 합니다</div>}
                </div>
              )}
            </div>
          </form>
